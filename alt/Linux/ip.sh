...
add_accept () { #Add the accepted IP to the Accept list
  iptables -A INPUT  -p tcp -s $1 -m owner --uid-owner $UID tcp --dport 443 -j ACCEPT
  iptables -A INPUT  -p tcp -s $1 -m owner --uid-owner $UID tcp --dport 80 -j ACCEPT
  iptables -A OUTPUT -p tcp -d $1 -m owner --uid-owner $UID tcp --dport 80 -j ACCEPT
  iptables -A OUTPUT -p tcp -d $1 -m owner --uid-owner $UID tcp --dport 443 -j ACCEPT
...
# Drop everything
iptables -m owner --uid-owner $UID -P OUTPUT  DROP
iptables -m owner --uid-owner $UID -P INPUT   DROP
iptables -m owner --uid-owner $UID -P FORWARD DROP

# Drop everything IPv6
ip6tables -m owner --uid-owner $UID -P OUTPUT  DROP
ip6tables -m owner --uid-owner $UID -P INPUT   DROP
ip6tables -m owner --uid-owner $UID -P FORWARD DROP
...
iptables -m owner --uid-owner $UID -A INPUT -i lo -j ACCEPT
iptables -m owner --uid-owner $UID -A OUTPUT -o lo -j ACCEPT
iptables -m owner --uid-owner $UID -A FORWARD -o lo -j ACCEPT
